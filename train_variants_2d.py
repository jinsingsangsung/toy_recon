...
def main():
    ...
    parser.add_argument('--use_pos', action='store_true', help='use position encoding')
...
def train(local_rank, args):
    ...
    if args.variant == "a":
        model = VariantsA(model=args.model, in_channels=3, out_channels=8, kernel_size=8, use_pos=args.use_pos)
    elif args.variant == "b":
        model = VariantsB(model=args.model, in_channels=3, out_channels=8, kernel_size=8, use_pos=args.use_pos)
    elif args.variant == "c": # or args.variant == "d":
        model = VariantsB(model=args.model, in_channels=3, out_channels=8, kernel_size=8, use_pos=args.use_pos)
    else:
        print("working on Vanilla model")
        model = Vanilla(model=args.model, in_channels=3, out_channels=8, kernel_size=8, use_pos=args.use_pos)
